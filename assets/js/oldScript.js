...
function getPronunciation() {
  fetch(wordsUrl0, options)
    ...
    .then((data) => {
      ...
      document.getElementById("mainPro").addEventListener('click', function (event) {
        var textToSpeech = new SpeechSynthesisUtterance();
        textToSpeech.text = word;
        window.speechSynthesis.speak(textToSpeech)
      })
      document.getElementById("mainPro").classList.add('tooltip')
      var tooltip2El = document.createElement('span')
      document.getElementById("mainPro").appendChild(tooltip2El)
      tooltip2El.classList.add('tooltiptext')
      tooltip2El.textContent = 'Click to hear pronunciation!'
    })
    .catch((err) => {
      console.error(err);
        ...
        function getPronunciation1() {
          fetch(word1Url0, options)
            ...
            .then((data) => {
              ...
              document.getElementById("modalPro").addEventListener('click', function (event) {
                var textToSpeech = new SpeechSynthesisUtterance();
                textToSpeech.text = word1;
                window.speechSynthesis.speak(textToSpeech)
              })
              document.getElementById("modalPro").classList.add('tooltip')
              var tooltip1El = document.createElement('span')
              document.getElementById("modalPro").appendChild(tooltip1El)
              tooltip1El.classList.add('tooltiptext')
              tooltip1El.setAttribute('id', 'tooltipstyle')
              tooltip1El.textContent = 'Click to hear pronunciation!'
            })
            .catch((err) => {
              console.error(err);
